...
function BundleConfirmContent() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [svcData, stgData, bdlData] = await Promise.all([
        ...
        // Look up eligible staff for each service via their allowedStaff
  ...
  // Per-service assignment preview
  ...
  }, [orderedSchedule, services, staffByService, staffOverrides])

  // ─── Square card form setup ────────────────────────────────────────────────
  useEffect(() => {
    if (!isBookingEnabled || paymentMethod !== 'card') return
    let isMounted = true
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Tokenize card first if paying online
      let cardToken = null
  ...
  return (
    <div style={{
      ...
      <p style={{ margin: '0.5rem 0 0 0', fontSize: '0.9rem' }}>
        This bundle booking requires confirmation before your appointment is finalized.
