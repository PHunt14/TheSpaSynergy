...
import { FadeIn } from '../../components/ServiceCatalog'
...
export default function ProviderDetailPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([providersData, servicesData]) => {
        const foundProvider = providersData.providers?.find(v => v.vendorId === vendorId)
  ...
  useEffect(() => {
    if (provider) {
      document.title = `${provider.name} | The Spa Synergy`
  ...
  if (!provider) return <div style={{ padding: '2rem' }}>Provider not found</div>

  const team = teamData[vendorId]

  return (
    <div style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
      <Link href="/providers" style={{ color: 'var(--color-primary)', marginBottom: '2rem', display: 'inline-block' }}>
        ← Back to Our Team
      </Link>
      ...
      <FadeIn>
        <p style={{ color: 'var(--color-text-light)', fontSize: '1.1rem', marginBottom: '3rem' }}>
          {provider.description || 'Welcome to our professional services. We are dedicated to providing exceptional experiences tailored to your unique needs.'}
        </p>
      </FadeIn>
            ...
            {Object.entries(groupedServices).map(([category, data]) => (
              <FadeIn key={category}>
                <div style={{
                  ...
                  <Link href={`/services?category=${encodeURIComponent(category)}`} className="cta" style={{ display: 'inline-block', fontSize: '0.9rem', padding: '0.6rem 1.2rem' }}>
                    Book Now
                  </Link>
                </div>
              </FadeIn>
        ...
        <div style={{
          ...
          {provider.phone && (
            <p style={{ fontSize: '1.1rem', marginBottom: '0.5rem' }}>
              <strong>Phone:</strong>{' '}
              <a href={`tel:${provider.phone}`} style={{ color: 'var(--color-primary-dark)', textDecoration: 'none' }}>{provider.phone}</a>
            </p>
          )}
          {provider.email && (
            <p style={{ fontSize: '1.1rem', marginBottom: '0.5rem' }}>
              <strong>Email:</strong>{' '}
              <a href={`mailto:${provider.email}`} style={{ color: 'var(--color-primary-dark)', textDecoration: 'none' }}>{provider.email}</a>
            </p>
          )}
          {(provider.socialFacebook || provider.socialInstagram || provider.socialTiktok || provider.socialWebsite || provider.googlePlaceId) && (
            <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
              {provider.googlePlaceId && (
                <a href={`https://search.google.com/local/writereview?placeid=${provider.googlePlaceId}`} target="_blank" rel="noopener noreferrer"
              ...
              {provider.socialFacebook && (
                <a href={provider.socialFacebook} target="_blank" rel="noopener noreferrer"
                  style={{ color: 'var(--color-primary-dark)', textDecoration: 'none', fontSize: '1rem' }}>Facebook</a>
              )}
              {provider.socialInstagram && (
                <a href={provider.socialInstagram} target="_blank" rel="noopener noreferrer"
                  style={{ color: 'var(--color-primary-dark)', textDecoration: 'none', fontSize: '1rem' }}>Instagram</a>
              )}
              {provider.socialTiktok && (
                <a href={provider.socialTiktok} target="_blank" rel="noopener noreferrer"
                  style={{ color: 'var(--color-primary-dark)', textDecoration: 'none', fontSize: '1rem' }}>TikTok</a>
              )}
              {provider.socialWebsite && (
                <a href={provider.socialWebsite} target="_blank" rel="noopener noreferrer"
