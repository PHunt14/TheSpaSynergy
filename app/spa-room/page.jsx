...
export default function SpaRoomPage() {
  ...
  useEffect(() => {
    fetch('/api/services')
      .then(r => r.json())
      .then(data => {
        const room = (data.services || []).filter(s => s.isActive !== false && s.resourceType === 'room')
        setServices(room)
        setLoading(false)
      })
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <h1 style={{ textAlign: 'center' }}>Book the Spa Room</h1>
      <p style={{ color: 'var(--color-text-light)', textAlign: 'center', marginBottom: '2rem' }}>
        Private spa room experiences for ultimate relaxation and rejuvenation. Select a service below to book.
      </p>
