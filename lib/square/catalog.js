...
export function buildCategoryObject(categoryName) {
  return {
    type: 'CATEGORY',
    id: `#category-${categoryName.toLowerCase().replaceAll(/\s+/g, '-')}`,
    categoryData: {
      name: categoryName,
    },
  }
...
export function buildUpsertBatches(services, existingCatalog) {
  const { items = new Map(), categories = new Map() } = existingCatalog || {}
  const objects = []
  const categoryGroups = groupByCategory(services)
  const categoryIdMap = new Map()

  for (const [catName] of categoryGroups) {
    const existingId = categories.get(catName)
    if (existingId) {
  ...
  for (const svc of services) {
    ...
    const existingItemId = items.get(svc.serviceId)
    if (existingItemId) {
...
export function buildOrderLineItems(services, people = 1) {
  return services.map(svc => ({
    name: svc.name,
    quantity: String(people || 1),
    basePriceMoney: {
