...
export function validateVendorForPayment(vendor) {
  if (!vendor) return { error: 'Vendor not found', status: 404 }
  if (vendor.squareOAuthStatus === 'error') {
    return { error: 'Payment unavailable', details: 'Vendor Square account needs to be reconnected', status: 400 }
  }
  const accessToken = vendor.squareAccessToken
  if (!accessToken) {
    return { error: 'Payment configuration error', details: 'Square payment not configured for this vendor', status: 500 }
  }
  return { accessToken, locationId: vendor.squareLocationId }
}
